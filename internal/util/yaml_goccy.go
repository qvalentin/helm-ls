...
func GetNodeForPosition(node ast.Node, position protocol.Position) ast.Node {
	// Apply Windows-specific position correction
	// This compensates for known Windows newline handling differences
	// as documented in internal/adapter/yamlls/diagnostics.go
	adjustedPosition := position
	if runtime.GOOS == "windows" {
		// Empirical offset correction for Windows goccy position reporting
		if position.Character >= 6 {
			adjustedPosition.Character = position.Character - 6
		}
	}
